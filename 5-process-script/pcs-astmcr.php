...
function hapus_data($data) {
    ...
    $get_query = "SELECT periode_laporan, rig_operation, rig_yard, sn_mcr, type, man, status_mcr, username FROM tb_dbhasilmcr WHERE id_mcr = '$id_mcr';";
    ...
    if ($record) {
        ...
        $sn_mcr = $record['sn_mcr'];
        $type = $record['type'];
        $man = $record['man'];
        $status = $record['status_mcr'];
        $inputed_by = $record['username'];
        $deleted_by = $_SESSION['username'] ?? 'unknown'; // Get the username from session

        $log_query = "
            INSERT INTO tb_dbdelmcr (deleted_item_id, periode_laporan, rig_operation, rig_yard, sn_mcr, type, man, status_mcr, inputed_by, deleted_by, deletion_time) 
            VALUES ('$id_mcr', '$periode_laporan', '$rig_operation', '$rig_yard', '$sn_mcr', '$type', '$man', '$status', '$inputed_by', '$deleted_by', CONVERT_TZ(NOW(), '+00:00', '+07:00'));
        ";
        mysqli_query($conn, $log_query);
    }

    // Step 3: Perform the deletion
    $delete_query = "DELETE FROM tb_dbhasilmcr WHERE id_mcr = '$id_mcr';";
...
if (isset($_GET['hapus'])) {
    ...
    header("location: page-astmcrdtb.php");
...
if (isset($_POST['aksi']) && $_POST['aksi'] === 'edit') {
    ...
    $type = $_POST['type'];
    $man = $_POST['man'];
    $status = $_POST['status_mcr'];
    $no_asset = $_POST['no_asset'];
    $no_mov = $_POST['no_mov'];
    $no_po = $_POST['no_po'];

    // Retrieve the original data
    $get_query = "SELECT * FROM tb_dbhasilmcr WHERE id_mcr = '$id_mcr'";
    $get_result = mysqli_query($conn, $get_query);
    
    if ($get_result && mysqli_num_rows($get_result) > 0) {
        $record = mysqli_fetch_assoc($get_result);

        // Insert the original data into tb_dbeditmcr
        ...
        if ($record['rig_operation'] !== $rigopr || $record['rig_yard'] !== $rigyard) {
            $historyQuery = "INSERT INTO tb_dbeditmcr 
                (edited_item_id, periode_laporan, rig_operation, rig_yard, sn_mcr, type, man, status_mcr, inputed_by, edited_by, edit_time, new_rig_operation, new_rig_yard)
                VALUES 
                ('$id_mcr', '{$record['periode_laporan']}', '{$record['rig_operation']}', '{$record['rig_yard']}', '{$record['sn_mcr']}', 
                 '{$record['type']}', '{$record['man']}', '{$record['status_mcr']}', '$inputed_by', '$edited_by', CONVERT_TZ(NOW(), '+00:00', '+07:00'), '$rigopr', '$rigyard')";
    ...
    // Update query
    $query = "UPDATE tb_dbhasilmcr
              SET periode_laporan = '$periode_laporan', rig_operation = '$rigopr', rig_yard = '$rigyard', sn_mcr = '$sn_mcr', type = '$type', man = '$man', status_mcr = '$status', no_asset = '$no_asset', no_mov = '$no_mov', no_po = '$no_po', username = '$username'
              WHERE id_mcr = '$id_mcr'";
...
if (isset($_POST['aksi']) && $_POST['aksi'] === 'add') {
    ...
    // Update query
    $query = "INSERT INTO tb_dbhasilmcr
              SET periode_laporan = '$periode_laporan', rig_operation = '$rigopr', rig_yard = '$rigyard', sn_mcr = '$sn_mcr', type = '$type', man = '$man', status_mcr = '$status', no_asset = '$no_asset', no_mov = '$no_mov', no_po = '$no_po', username = '$username'";
