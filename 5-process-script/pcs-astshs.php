...
function hapus_data($data) {
    ...
    $get_query = "SELECT periode_laporan, rig_operation, rig_yard, sn_shs, type, man, status_shs, username FROM tb_dbhasilshs WHERE id_shs = '$id_shs';";
    ...
    if ($record) {
        ...
        $sn_shs = $record['sn_shs'];
        $type = $record['type'];
        $man = $record['man'];
        $status = $record['status_shs'];
        $inputed_by = $record['username'];
        $deleted_by = $_SESSION['username'] ?? 'unknown'; // Get the username from session

        $log_query = "
            INSERT INTO tb_dbdelshs (deleted_item_id, periode_laporan, rig_operation, rig_yard, sn_shs, type, man, status_shs, inputed_by, deleted_by, deletion_time) 
            VALUES ('$id_shs', '$periode_laporan', '$rig_operation', '$rig_yard', '$sn_shs', '$type', '$man', '$status', '$inputed_by', '$deleted_by', CONVERT_TZ(NOW(), '+00:00', '+07:00'));
        ";
        mysqli_query($conn, $log_query);
    }

    // Step 3: Perform the deletion
    $delete_query = "DELETE FROM tb_dbhasilshs WHERE id_shs = '$id_shs';";
...
if (isset($_GET['hapus'])) {
    ...
    header("location: page-astshsdtb.php");
...
if (isset($_POST['aksi']) && $_POST['aksi'] === 'edit') {
    ...
    $type = $_POST['type'];
    $man = $_POST['man'];
    $status = $_POST['status_shs'];
    $no_asset = $_POST['no_asset'];
    $no_mov = $_POST['no_mov'];
    $no_po = $_POST['no_po'];

    // Retrieve the original data
    $get_query = "SELECT * FROM tb_dbhasilshs WHERE id_shs = '$id_shs'";
    $get_result = mysqli_query($conn, $get_query);
    
    if ($get_result && mysqli_num_rows($get_result) > 0) {
        $record = mysqli_fetch_assoc($get_result);

        // Insert the original data into tb_dbeditshs
        ...
        if ($record['rig_operation'] !== $rigopr || $record['rig_yard'] !== $rigyard) {
            $historyQuery = "INSERT INTO tb_dbeditshs 
                (edited_item_id, periode_laporan, rig_operation, rig_yard, sn_shs, type, man, status_shs, inputed_by, edited_by, edit_time, new_rig_operation, new_rig_yard)
                VALUES 
                ('$id_shs', '{$record['periode_laporan']}', '{$record['rig_operation']}', '{$record['rig_yard']}', '{$record['sn_shs']}', 
                 '{$record['type']}', '{$record['man']}', '{$record['status_shs']}', '$inputed_by', '$edited_by', CONVERT_TZ(NOW(), '+00:00', '+07:00'), '$rigopr', '$rigyard')";
    ...
    // Update query
    $query = "UPDATE tb_dbhasilshs
              SET periode_laporan = '$periode_laporan', rig_operation = '$rigopr', rig_yard = '$rigyard', sn_shs = '$sn_shs', type = '$type', man = '$man', status_shs = '$status', no_asset = '$no_asset', no_mov = '$no_mov', no_po = '$no_po', username = '$username'
              WHERE id_shs = '$id_shs'";
...
if (isset($_POST['aksi']) && $_POST['aksi'] === 'add') {
    ...
    // Update query
    $query = "INSERT INTO tb_dbhasilshs
              SET periode_laporan = '$periode_laporan', rig_operation = '$rigopr', rig_yard = '$rigyard', sn_shs = '$sn_shs', type = '$type', man = '$man', status_shs = '$status', no_asset = '$no_asset', no_mov = '$no_mov', no_po = '$no_po', username = '$username'";
