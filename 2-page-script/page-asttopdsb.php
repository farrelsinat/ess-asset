...
<head>
  ...
  <title>ESS | TOP DRIVE Dashboard</title>
...
<body>
  ...
  <nav class="navbar navbar-second fixed-top" style="background-color: #b02a37;">
    <div class="container-fluid justify-content-center">
      <a class="navbar-brand text-white">DASHBOARD - TOP DRIVE</a>
    </div>
  </nav>
...
?>
  ...
  foreach ($jawas as $jawa) {
      $sql = "SELECT COUNT(*) AS total FROM tb_dbhasiltop 
  ...
  foreach ($wsvendors as $wsvendor) {
      $sql = "SELECT COUNT(*) AS total FROM tb_dbhasiltop 
              WHERE rig_operation = '$wsvendor'";
      ...
      // Fetch the count from the result
      $row = $result->fetch_assoc();
      $wsvendorCounts[$wsvendor] = $row['total'] ?? 0; // Default to 0 if no result
  }
...
try {
    ...
    function getTopCounts($pdo, $rigopr, $rigyards) {
    ...
    $jawaRigyards = ["PDSI #08.1/H40D-M", "PDSI #09.2/N80UE-E", "PDSI #12.3/N110-M", "PDSI #13.1/H40D-M", "PDSI #15.3/N110-M", "PDSI #28.2/D1000-E", "PDSI #31.3/D1500-E", "PDSI #38.2/D1000-E", "PDSI #40.4/DS2000-E", "GUDANG 4 MUNDU", "YARD BONGAS", "YARD KAPETAKAN", "IDTC"];
    ...
    $wsvendorRigyards = ["NOV BSD"];

    // Fetch counts for each category
    $jawCounts = getTopCounts($pdo, 'JAWA', $jawaRigyards);
    $sbCounts = getTopCounts($pdo, 'SBS', $sbsRigyards);
    $janarCounts = getTopCounts($pdo, 'JANARO', $janaroRigyards);
    $ktCounts = getTopCounts($pdo, 'KTI', $ktiRigyards);
    $offshorCounts = getTopCounts($pdo, 'OFF SHORE', $offshoreRigyards);
    $wsvenCounts = getTopCounts($pdo, 'WS VENDOR', $wsvendorRigyards);
...
?>
  ...
  <div class="container dashboard-section">
    <div class="col">
      <div class="card dashboard-card text-center mb-0" style="width: 100%; border-radius: 15px; box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.2); padding: 5px 5px;">
        ...
        <div class="d-flex justify-content-center align-items-center mt-0">
          <h5 class="count-box text-danger mb-0" style="font-size: 4rem; font-weight: bold;"><?php echo $total_top; ?></h5>
        </div>
        <div class="button-container mt-2 mb-2">
              <a href="page-asttopfinput.php" type="button" class="btn btn-success" style="width: 100%; font-weight: bold;">
                <i class="fa fa-plus-square me-2" aria-hidden="true"></i>Input Data
              </a>
          <a href="page-asttopdtb.php" type="button" class="btn btn-warning" style="width: 100%; font-weight: bold;">
  ...
  <div class="container dashboard-section">
    <div class="row">
      ...
      <div class="col-md-3">
        <div class="card mt-3 dashboard-card text-center" style="width: 100%; box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.2); border-radius: 15px;">
          <div class="card-body">
            <p class="card-title mb-0" style="font-size: 0.9rem; font-weight: bold;"><?php echo $wsvendor; ?></p>
            <p class="card-text text-success mb-0" style="font-size: 2rem; font-weight: bold;"><?php echo $wsvendorCounts[$wsvendor]; ?></p>
            <a href="#" class="btn btn-primary mt-0" style="color: white;" data-bs-toggle="modal" data-bs-target="#wsvendorModal"><strong>Detail</strong></a>
          </div>
        </div>
      </div>
  ...
  <script>
    document.querySelectorAll('.detail-btn').forEach(button => {
        button.addEventListener('click', function () {
            ...
            fetch(`fct-asttopdsb.php?rig_yard=${encodedRigYard}&rig_operation=${rig_operation}`)
  ...
  <!-- WS VENDOR Modal -->
  <div class="modal fade" id="wsvendorModal" tabindex="-1" aria-labelledby="wsvendorModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="wsvendorModalLabel">WS VENDOR</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
              <div class="row">
                  <?php if (!empty($wsvenCounts)) : ?>
                      <table class="table table-bordered">
                          <thead>
                              <tr>
                                  <th class="text-center" style="width: 50%;">Letak</th>
                          ...
                          <tbody>
                              <?php foreach ($wsvenCounts as $count) : ?>
                                  <tr>
                                      <td class="text-center" style="width: 50%;"><?= $count['title']; ?></td>
                                      <td class="text-center" style="width: 50%;">
                                          <button 
                                              class="btn btn-link detail-btn" 
                                              data-bs-toggle="modal" 
                                              data-bs-target="#wsvendordetailModal" 
                                              data-rigyard="<?= htmlspecialchars($count['title'], ENT_QUOTES); ?>"
                                              data-rigopr="WS VENDOR">
                                              <?= $count['count']; ?>
                                          </button>
                                      </td>
                                  </tr>
  ...
  <?php if ($user_role == 'admin' || $user_role == 'asset' || $user_role == 'maintenance' || $user_role == 'operation') : ?>
  <div class="modal fade" id="wsvendordetailModal" tabindex="-1" aria-labelledby="wsvendorModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" id="wsvendordetailModalLabel"></h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                  <div id="wsvendordetailContent">
                      <!-- Details will be loaded here -->
                      <p>Loading...</p>
                  </div>
              </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        detailButtons.forEach(button => {
            button.addEventListener("click", function () {
                ...
                // Update the modal title
                const wsvendordetailModalLabel = document.getElementById("wsvendordetailModalLabel");
                wsvendordetailModalLabel.textContent = `WS VENDOR - ${rig_yard}`;
  ...
  <script>
    document.querySelectorAll('.detail-btn').forEach(button => {
        button.addEventListener('click', function () {
            const rig_yard = this.getAttribute('data-rigyard');
            const rig_operation = this.getAttribute('data-rigopr');
            const modalContent = document.getElementById('wsvendordetailContent');
