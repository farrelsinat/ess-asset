...
<head>
  ...
  <title>ESS | TOP DRIVE Database</title>
...
<body>
  ...
  <nav class="navbar navbar-second fixed-top" style="background-color: #ffc107;">
    <div class="container-fluid justify-content-center">
      <a class="navbar-brand text">DATABASE - TOP DRIVE</a>
    </div>
  </nav>
        ...
        <div class="modal-content p-3">
            ...
            <div class="d-flex flex-column align-items-center">
                <div class="card mt-3" style="width: 90%; border-width: 1px; padding: 10px;">
                    ...
                    <p class="text">Database ini adalah seluruh data unit TOP DRIVE berdasarkan update pada 23 Maret 2023.</p>
                    <p class="text">Dimungkinkan ada perubahan atau tambahan data unit TOP DRIVE yang belum tercatat, mohon diperiksa kembali setiap data tersebut.</p>
                    <p class="text">Apabila menemukan data unit TOP DRIVE yang terduplikasi(double), harap menghubungi PIC.</p>
                </div>
                <div class="card mt-3" style="width: 90%; border-width: 1px; padding: 10px;">
                    <h6 class="text">CARA PENGGUNAAN</h6>
                    <p class="text">Input Data = Form untuk menambahkan data unit TOP DRIVE pengadaan/investasi baru dan/atau data unit TOP DRIVE yang belum pernah masuk ke dalam database.</p>
                    <p class="text">Dashboard = Rangkuman data sebaran unit TOP DRIVE di dalam database.</p>
                    <p class="text">Riwayat Edit = Informasi terkait perubahan(lokasi, dll) pada data unit TOP DRIVE.</p>
                    <p class="text">Riwayat Hapus = Informasi terkait penghapusan suatu data unit TOP DRIVE.</p>
                    <p class="text">
                        <button type="button" class="btn btn-primary btn-sm btn-xs">
                            <i class="fa fa-pencil-square" aria-hidden="true"></i>
                        </button> = Form untuk mengubah data unit TOP DRIVE, seperti perubahan pada lokasi, COC, dll.
                    </p>
                    <p class="text">
                        <button type="button" class="btn btn-danger btn-sm btn-xs">
                            <i class="fa fa-trash" aria-hidden="true"></i>
                        </button> = Tombol untuk menghapus suatu data unit TOP DRIVE.
                    </p>
                </div>
            </div>
        </div>
  ...
  <div class="container">
    <div class="card dashboard-card row mt-5">
      ...
      <div class="button-container mt-3">
        <?php if ($user_role == 'admin' || $user_role == 'asset') : ?>
            <a href="page-asttopfinput.php" class="btn btn-success" style="width: 100%; font-weight: bold;">
        ...
        <?php endif; ?>
        <a href="page-asttopdsb.php" type="button" class="btn btn-primary" style="width: 100%; font-weight: bold;">
            ...
            <tr class="align-middle text-center">
              ...
              <td>
                <?php if ($user_role == 'admin' || $user_role == 'asset' || $user_role == 'maintenance') : ?>
                    <a href="page-asttopfedit.php?edit=<?php echo $result['id_top'] ?>&sn_top=<?php echo $result['sn_top'] ?>" type="button" class="btn btn-primary btn-sm btn-xs">
                      <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                ...
                <?php endif; ?>
                <?php if ($user_role == 'admin' || $user_role == 'asset') : ?>
                    <a href="pcs-asttop.php?hapus=<?php echo $result['id_top'] ?>" type="button" class="btn btn-danger btn-sm btn-xs" onClick="return confirm('Apakah anda yakin ingin menghapus data ini?')">
                      <i class="fa fa-trash" aria-hidden="true"></i>
                    </a>
                <?php elseif ($user_role == 'maintenance' || $user_role == 'operation' || $user_role == 'guest') : ?>
              ...
              <td>
                <?php echo $result['tonase'] ?>
              </td>
              <td>
                <?php echo $result['tahun_beli'] ?>
              </td>
              <td>
                <?php echo $result['status_top'] ?>
              </td>
            </tr>
            <?php
          }
    ...
    document.getElementById("exportExcel").addEventListener("click", function () {
        ...
        XLSX.writeFile(wb, "TopDrive-ESS-2025.xlsx");
    });
    ...
    document.getElementById("exportCSV").addEventListener("click", function () {
        ...
        link.download = "TopDrive-ESS-2025.csv";
        link.click();
    });
    </script>
  ...
  <div class="modal fade" id="deltableModal" tabindex="-1" aria-labelledby="deltableModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="deltableModalLabel">RIWAYAT HAPUS - TOP DRIVE</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
            ...
            <div class="table-responsive">
              <table id="dz" class="table table-striped table-tertiary-color cell-border hover text-white" style="width: 100%;">
                ...
                <tbody>
                  ...
                  $sql = "SELECT periode_laporan, rig_operation, rig_yard, sn_top, type, tonase, status_top, inputed_by, deleted_by, deletion_time FROM tb_dbdeltop";
                  ...
                  while ($row = $query->fetch_assoc()){
                    ...
                    <tr class="align-middle text-center">
                      ...
                      <td><?php echo $row['sn_top'];?></td>
                      <td><?php echo $row['type'];?></td>
                      <td><?php echo $row['tonase'];?></td>
                      <td><?php echo $row['status_top'];?></td>
    ...
    <div class="modal fade" id="edittableModal" tabindex="-1" aria-labelledby="edittableModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="edittableModalLabel">RIWAYAT EDIT - TOP DRIVE</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <!-- Table Inside Modal -->
            <div class="table-responsive">
              <table id="ds" class="table table-striped table-tertiary-color cell-border hover text-white" style="width: 100%;">
                ...
                <tbody>
                  ...
                  $sql = "SELECT periode_laporan, rig_operation, rig_yard, new_rig_operation, new_rig_yard, sn_top, type, tonase, status_top, inputed_by, edited_by, edit_time FROM tb_dbedittop";
                  $query = $conn->query($sql);
                  while ($row = $query->fetch_assoc()){
                    ?>
                    <tr class="align-middle text-center">
