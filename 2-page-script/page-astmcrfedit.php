...
<head>
  ...
  <title>ESS | MUD CLEANER Edit Data Lokasi</title>
...
<body>
  ...
  <nav class="navbar navbar-second fixed-top" style="background-color: #0d6efd;">
    <div class="container-fluid justify-content-center">
      <a class="navbar-brand text-white">EDIT DATA - MUD CLEANER</a>
    </div>
  </nav>
    ...
    <div class="mb-3 row justify-content-center">
      ...
      <form id="dataForm" method="POST" action="pcs-astmcr">
          ...
          <div class="mb-3 row justify-content-center">
              ...
              <div class="col-sm-5">
                  <input type="text" class="form-control" id="sn_mcr" name="sn_mcr" readonly value="<?php echo $snmcr; ?>">
              </div>
          </div>
          ...
          <div class="mb-3 row justify-content-center">
              ...
              <div class="col-sm-5">
                  <input type="text" class="form-control" id="status_mcr" name="status_mcr">
              </div>
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
        const snMCRField = document.getElementById('sn_mcr');
        if (snMCRField.value) {
            fetchData(snMCRField.value);
        }

        snMCRField.addEventListener('input', function () {
            fetchData(this.value);
        });

        function fetchData(snMCR) {
            if (snMCR) {
                fetch(`fct-astmcrfedit.php?sn_mcr=${snMCR}`)
                    .then(response => response.json())
                    .then(data => {
                        document.getElementById('type').value = data.type || '';
                        document.getElementById('man').value = data.man || '';
                        document.getElementById('status_mcr').value = data.status_mcr || '';
