...
<head>
  ...
  <title>ESS | TUBULAR Database</title>
...
<body>
  ...
  <nav class="navbar navbar-second fixed-top" style="background-color: #ffc107;">
    <div class="container-fluid justify-content-center">
      <a class="navbar-brand text">DATABASE - TUBULAR</a>
    </div>
  </nav>
        ...
        <div class="modal-content p-3">
            ...
            <div class="d-flex flex-column align-items-center">
                <div class="card mt-3" style="width: 90%; border-width: 1px; padding: 10px;">
                    ...
                    <p class="text">Database ini adalah seluruh data TUBULAR berdasarkan update pada 10 Januari 2025.</p>
                    <p class="text">Dimungkinkan ada perubahan atau tambahan data TUBULAR yang belum tercatat, mohon diperiksa kembali setiap data tersebut.</p>
                    <p class="text">Apabila menemukan data TUBULAR yang terduplikasi(double), harap menghubungi PIC.</p>
                </div>
                <div class="card mt-3" style="width: 90%; border-width: 1px; padding: 10px;">
                    <h6 class="text">CARA PENGGUNAAN</h6>
                    <p class="text">Input Data = Form untuk menambahkan data TUBULAR pengadaan/investasi baru dan/atau data TUBULAR yang belum pernah masuk ke dalam database.</p>
                    <p class="text">Dashboard = Rangkuman data sebaran TUBULAR di dalam database.</p>
                    <p class="text">Riwayat Edit = Informasi terkait perubahan(lokasi, dll) pada data TUBULAR.</p>
                    <p class="text">Riwayat Hapus = Informasi terkait penghapusan suatu data TUBULAR.</p>
                    <p class="text"><button type="button" class="btn btn-primary btn-sm btn-xs">
                      <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </button> = Form untuk mengubah data TUBULAR, seperti perubahan pada lokasi, COC, dll.</p>
                    <p class="text"><button type="button" class="btn btn-danger btn-sm btn-xs">
                      <i class="fa fa-trash" aria-hidden="true"></i>
                    </button> = Tombol untuk menghapus suatu data TUBULAR.</p>
                </div>
            </div>
        </div>
    </div>
  ...
  <div class="container">
    <div class="card dashboard-card row mt-5">
      ...
      <div class="button-container mt-3">
        ...
        <?php elseif ($user_role == 'maintenance' || $user_role == 'operation' || $user_role == 'guest') : ?>
            <button type="button" class="btn btn-success" style="width: 100%; font-weight: bold;" disabled>
              <i class="fa fa-plus-square" aria-hidden="true"></i> Input Data
            </button>
        <?php endif; ?>
        <a href="page-asttubgoddsb.php" type="button" class="btn btn-primary" style="width: 100%; font-weight: bold;">
        ...
        <button type="button" class="btn btn-danger" style="width: 100%; font-weight: bold;" data-bs-toggle="modal" data-bs-target="#deltableModal">
            ...
            <tr class="align-middle text-center">
              ...
              <td>
                <?php if ($user_role == 'admin' || $user_role == 'asset' || $user_role == 'maintenance') : ?>
                    <a href="page-asttubgodfedit.php?edit=<?php echo $result['id_tubgod'] ?>&sn_pin=<?php echo $result['sn_pin'] ?>" type="button" class="btn btn-primary btn-sm btn-xs">
                      <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                ...
                <?php endif; ?>
                <?php if ($user_role == 'admin' || $user_role == 'asset') : ?>
                    <a href="pcs-asttubgod.php?hapus=<?php echo $result['id_tubgod'] ?>" type="button" class="btn btn-danger btn-sm btn-xs" onClick="return confirm('Apakah anda yakin ingin menghapus data ini?')">
                      <i class="fa fa-trash" aria-hidden="true"></i>
                    </a>
                <?php elseif ($user_role == 'maintenance' || $user_role == 'operation' || $user_role == 'guest') : ?>
              ...
              <td>
                <?php echo $result['sn_pin'] ?>
              </td>
              <td>
                <?php echo $result['sn_box'] ?>
              </td>
              <td>
                <?php echo $result['no_mov'] ?>
              </td>
              <td>
                <?php echo $result['jenis_tubgod'] ?>
              </td>
              <td>
                <?php echo $result['man'] ?>
              </td>
              <td>
                <?php echo $result['conn_type'] ?>
              </td>
              <td>
                <?php echo $result['pounder'] ?>
              </td>
              <td>
    ...
    document.getElementById("exportExcel").addEventListener("click", function () {
        ...
        XLSX.writeFile(wb, "TubularGoods-ESS-2025.xlsx");
    });
    ...
    document.getElementById("exportCSV").addEventListener("click", function () {
        ...
        link.download = "TubularGoods-ESS-2025.csv";
        link.click();
    });
    </script>
  ...
  <div class="modal fade" id="deltableModal" tabindex="-1" aria-labelledby="deltableModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="deltableModalLabel">RIWAYAT HAPUS - TUBULAR</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
            ...
            <div class="table-responsive">
              <table id="ds" class="table table-striped table-tertiary-color cell-border hover text-white" style="width: 100%;">
                ...
                <tbody>
                  ...
                  $sql = "SELECT periode_laporan, rig_operation, rig_yard, sn_pin, jenis_tubgod, inputed_by, deleted_by, deletion_time FROM tb_dbdeltubgod";
