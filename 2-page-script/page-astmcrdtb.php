...
<head>
  ...
  <title>ESS | MUD CLEANER Database</title>
...
<body>
  ...
  <nav class="navbar navbar-second fixed-top" style="background-color: #ffc107;">
    <div class="container-fluid justify-content-center">
      <a class="navbar-brand text">DATABASE - MUD CLEANER</a>
    </div>
  </nav>
        ...
        <div class="modal-content p-3">
            ...
            <div class="d-flex flex-column align-items-center">
                <div class="card mt-3" style="width: 90%; border-width: 1px; padding: 10px;">
                    ...
                    <p class="text">Database ini adalah seluruh data unit MUD CLEANER berdasarkan update pada 30 April 2025.</p>
                    <p class="text">Dimungkinkan ada perubahan atau tambahan data unit MUD CLEANER yang belum tercatat, mohon diperiksa kembali setiap data tersebut.</p>
                    <p class="text">Apabila menemukan data unit MUD CLEANER yang terduplikasi(double), harap menghubungi PIC.</p>
                </div>
                <div class="card mt-3" style="width: 90%; border-width: 1px; padding: 10px;">
                    <h6 class="text">CARA PENGGUNAAN</h6>
                    <p class="text">Input Data = Form untuk menambahkan data unit MUD CLEANER pengadaan/investasi baru dan/atau data unit MUD CLEANER yang belum pernah masuk ke dalam database.</p>
                    <p class="text">Dashboard = Rangkuman data sebaran unit MUD CLEANER di dalam database.</p>
                    <p class="text">Riwayat Edit = Informasi terkait perubahan(lokasi, dll) pada data unit MUD CLEANER.</p>
                    <p class="text">Riwayat Hapus = Informasi terkait penghapusan suatu data unit MUD CLEANER.</p>
                    <p class="text">
                        <button type="button" class="btn btn-primary btn-sm btn-xs">
                            <i class="fa fa-pencil-square" aria-hidden="true"></i>
                        </button> = Form untuk mengubah data unit MUD CLEANER, seperti perubahan pada lokasi, COC, dll.
                    </p>
                    <p class="text">
                        <button type="button" class="btn btn-danger btn-sm btn-xs">
                            <i class="fa fa-trash" aria-hidden="true"></i>
                        </button> = Tombol untuk menghapus suatu data unit MUD CLEANER.
                    </p>
                </div>
            </div>
        </div>
  ...
  <div class="container">
    <div class="card dashboard-card row mt-5">
      ...
      <div class="button-container mt-3">
        <?php if ($user_role == 'admin' || $user_role == 'asset') : ?>
            <a href="page-astmcrfinput.php" class="btn btn-success" style="width: 100%; font-weight: bold;">
        ...
        <?php endif; ?>
        <a href="page-astmcrdsb.php" type="button" class="btn btn-primary" style="width: 100%; font-weight: bold;">
            ...
            <tr class="align-middle text-center">
              ...
              <td>
                <?php if ($user_role == 'admin' || $user_role == 'asset' || $user_role == 'maintenance') : ?>
                    <a href="page-astmcrfedit.php?edit=<?php echo $result['id_mcr'] ?>&sn_mcr=<?php echo $result['sn_mcr'] ?>" type="button" class="btn btn-primary btn-sm btn-xs">
                      <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                ...
                <?php endif; ?>
                <?php if ($user_role == 'admin' || $user_role == 'asset') : ?>
                    <a href="pcs-astmcr.php?hapus=<?php echo $result['id_mcr'] ?>" type="button" class="btn btn-danger btn-sm btn-xs" onClick="return confirm('Apakah anda yakin ingin menghapus data ini?')">
                      <i class="fa fa-trash" aria-hidden="true"></i>
                    </a>
                <?php elseif ($user_role == 'maintenance' || $user_role == 'operation' || $user_role == 'guest') : ?>
    ...
    document.getElementById("exportExcel").addEventListener("click", function () {
        ...
        XLSX.writeFile(wb, "MudCleaner-ESS-2025.xlsx");
    });
    ...
    document.getElementById("exportCSV").addEventListener("click", function () {
        ...
        link.download = "MudCleaner-ESS-2025.csv";
        link.click();
    });
    </script>
  ...
  <div class="modal fade" id="deltableModal" tabindex="-1" aria-labelledby="deltableModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="deltableModalLabel">RIWAYAT HAPUS - MUD CLEANER</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
            ...
            <div class="table-responsive">
              <table id="dz" class="table table-striped table-tertiary-color cell-border hover text-white" style="width: 100%;">
                ...
                <tbody>
                  <?php
                  $server = "localhost";
                  $username = "u597806360_asset_mcr";
                  $password = "AsT-PdsI#2025_MCr";
                  $dbname = "u597806360_db_mcr";
                  ...
                  $sql = "SELECT periode_laporan, rig_operation, rig_yard, sn_mcr, type, man, status_mcr, inputed_by, deleted_by, deletion_time FROM tb_dbdelmcr";
                  ...
                  while ($row = $query->fetch_assoc()){
                    ...
                    <tr class="align-middle text-center">
                      ...
                      <td><?php echo $row['sn_mcr'];?></td>
                      <td><?php echo $row['type'];?></td>
                      <td><?php echo $row['man'];?></td>
                      <td><?php echo $row['status_mcr'];?></td>
    ...
    <div class="modal fade" id="edittableModal" tabindex="-1" aria-labelledby="edittableModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="edittableModalLabel">RIWAYAT EDIT - MUD CLEANER</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <!-- Table Inside Modal -->
            <div class="table-responsive">
              <table id="ds" class="table table-striped table-tertiary-color cell-border hover text-white" style="width: 100%;">
                ...
                <tbody>
                  ...
                  $sql = "SELECT periode_laporan, rig_operation, rig_yard, new_rig_operation, new_rig_yard, sn_mcr, type, man, status_mcr, inputed_by, edited_by, edit_time FROM tb_dbeditmcr";
                  $query = $conn->query($sql);
                  while ($row = $query->fetch_assoc()){
                    ?>
                    <tr class="align-middle text-center">
