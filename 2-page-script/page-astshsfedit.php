...
<head>
  ...
  <title>ESS | SHALE SHAKER Edit Data Lokasi</title>
...
<body>
  ...
  <nav class="navbar navbar-second fixed-top" style="background-color: #0d6efd;">
    <div class="container-fluid justify-content-center">
      <a class="navbar-brand text-white">EDIT DATA - SHALE SHAKER</a>
    </div>
  </nav>
    ...
    <div class="mb-3 row justify-content-center">
      ...
      <form id="dataForm" method="POST" action="pcs-astshs">
          ...
          <div class="mb-3 row justify-content-center">
              ...
              <div class="col-sm-5">
                  <input type="text" class="form-control" id="sn_shs" name="sn_shs" readonly value="<?php echo $snshs; ?>">
              </div>
          </div>
          ...
          <div class="mb-3 row justify-content-center">
              ...
              <div class="col-sm-5">
                  <input type="text" class="form-control" id="status_shs" name="status_shs">
              </div>
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
        const snSHSField = document.getElementById('sn_shs');
        if (snSHSField.value) {
            fetchData(snSHSField.value);
        }

        snSHSField.addEventListener('input', function () {
            fetchData(this.value);
        });

        function fetchData(snSHS) {
            if (snSHS) {
                fetch(`fct-astshsfedit.php?sn_shs=${snSHS}`)
                    .then(response => response.json())
                    .then(data => {
                        document.getElementById('type').value = data.type || '';
                        document.getElementById('man').value = data.man || '';
                        document.getElementById('status_shs').value = data.status_shs || '';
                        document.getElementById('no_mov').value = data.no_mov || '';
                        document.getElementById('no_asset').value = data.no_asset || '';
