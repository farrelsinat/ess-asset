...
<head>
  ...
  <title>ESS | MUD PUMP Edit Data Lokasi</title>
...
<body>
  ...
  <nav class="navbar navbar-second fixed-top" style="background-color: #0d6efd;">
    <div class="container-fluid justify-content-center">
      <a class="navbar-brand text-white">EDIT DATA - MUD PUMP</a>
    </div>
  </nav>
    ...
    <div class="mb-3 row justify-content-center">
      ...
      <form id="dataForm" method="POST" action="pcs-astmpp">
          ...
          <div class="mb-3 row justify-content-center">
              ...
              <div class="col-sm-5">
                  <input type="text" class="form-control" id="sn_mpp" name="sn_mpp" readonly value="<?php echo $snmpp; ?>">
              </div>
          </div>
          ...
          <div class="mb-3 row justify-content-center">
              ...
              <div class="col-sm-5">
                  <input type="text" class="form-control" id="status_mpp" name="status_mpp">
              </div>
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
        const snMPPField = document.getElementById('sn_mpp');
        if (snMPPField.value) {
            fetchData(snMPPField.value);
        }

        snMPPField.addEventListener('input', function () {
            fetchData(this.value);
        });

        function fetchData(snMPP) {
            if (snMPP) {
                fetch(`fct-astmppfedit.php?sn_mpp=${snMPP}`)
                    .then(response => response.json())
                    .then(data => {
                        document.getElementById('type').value = data.type || '';
                        document.getElementById('man').value = data.man || '';
                        document.getElementById('status_mpp').value = data.status_mpp || '';
