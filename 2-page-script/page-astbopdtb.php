...
<body>
  ...
  <nav class="navbar navbar-second fixed-top" style="background-color: #ffc107;">
    <div class="container-fluid justify-content-center">
      <a class="navbar-brand text">DATABASE - BOP</a>
    </div>
  </nav>
        ...
        <div class="modal-content p-3">
            ...
            <div class="d-flex flex-column align-items-center">
                <div class="card mt-3" style="width: 90%; border-width: 1px; padding: 10px;">
                    ...
                    <p class="text">Database ini adalah seluruh data unit BOP berdasarkan update pada 23 Januari 2025.</p>
                    <p class="text">Dimungkinkan ada perubahan atau tambahan data unit BOP yang belum tercatat, mohon diperiksa kembali setiap data tersebut.</p>
                    <p class="text">Apabila menemukan data unit BOP yang terduplikasi(double), harap menghubungi PIC.</p>
                </div>
                <div class="card mt-3" style="width: 90%; border-width: 1px; padding: 10px;">
                    <h6 class="text">CARA PENGGUNAAN</h6>
                    <p class="text">Input Data = Form untuk menambahkan data unit BOP pengadaan/investasi baru dan/atau data unit BOP yang belum pernah masuk ke dalam database.</p>
                    <p class="text">Dashboard = Rangkuman data sebaran unit BOP di dalam database.</p>
                    <p class="text">Riwayat Edit = Informasi terkait perubahan(lokasi, dll) pada data unit BOP.</p>
                    <p class="text">Riwayat Hapus = Informasi terkait penghapusan suatu data unit BOP.</p>
                    <p class="text">
                        <button type="button" class="btn btn-primary btn-sm btn-xs">
                            <i class="fa fa-pencil-square" aria-hidden="true"></i>
                        </button> = Form untuk mengubah data unit BOP, seperti perubahan pada lokasi, COC, dll.
                    </p>
                    <p class="text">
                        <button type="button" class="btn btn-danger btn-sm btn-xs">
                            <i class="fa fa-trash" aria-hidden="true"></i>
                        </button> = Tombol untuk menghapus suatu data unit BOP.
                    </p>
                </div>
            </div>
        </div>
  ...
  <div class="container">
    <div class="card dashboard-card row mt-5">
      ...
      <div class="button-container mt-3">
        <?php if ($user_role == 'admin' || $user_role == 'asset') : ?>
            <a href="page-astbopfinput.php" class="btn btn-success" style="width: 100%; font-weight: bold;">
        ...
        <?php endif; ?>
        <a href="page-astbopdsb.php" type="button" class="btn btn-primary" style="width: 100%; font-weight: bold;">
            ...
            <tr class="align-middle text-center">
              ...
              <td>
                <?php if ($user_role == 'admin' || $user_role == 'asset' || $user_role == 'maintenance') : ?>
                    <a href="page-astbopfedit.php?edit=<?php echo $result['id_bop'] ?>&sn_bop=<?php echo $result['sn_bop'] ?>" type="button" class="btn btn-primary btn-sm btn-xs">
                      <i class="fa fa-pencil-square" aria-hidden="true"></i>
                    </a>
                ...
                <?php endif; ?>
                <?php if ($user_role == 'admin' || $user_role == 'asset') : ?>
                    <a href="pcs-astbop.php?hapus=<?php echo $result['id_bop'] ?>" type="button" class="btn btn-danger btn-sm btn-xs" onClick="return confirm('Apakah anda yakin ingin menghapus data ini?')">
                      <i class="fa fa-trash" aria-hidden="true"></i>
                    </a>
                <?php elseif ($user_role == 'maintenance' || $user_role == 'operation' || $user_role == 'guest') : ?>
              ...
              <td>
                <?php echo $result['sn_bop'] ?>
              </td>
              <td>
                <?php echo $result['no_mov'] ?>
              </td>
              <td>
                <?php echo $result['jenis_bop'] ?>
              </td>
              <td>
                <?php echo $result['size'] ?>
              </td>
              <td>
                <?php echo $result['pressure'] ?>
              </td>
              <td>
                <?php echo $result['man'] ?>
              </td>
              ...
              <td>
                <?php echo $result['status_unit'] ?>
              </td>
              <td>
                <?php echo $result['no_coc'] ?>
              </td>
              <td>
                <?php echo $result['akhir_coc'] ?>
              </td>
              <td>
                <?php echo $result['status_coc'] ?>
              </td>
              <td>
                <?php if (!empty($result['file_coc'])): ?>
                    <?php if ($user_role == 'admin' || $user_role == 'asset' || $user_role == 'maintenance') : ?>
                        <a href="<?php echo $result['file_coc']; ?>" target="_blank" class="btn btn-success btn-sm btn-xs"><i class="fa fa-eye" aria-hidden="true"></i></a>
                        <a href="<?php echo $result['file_coc']; ?>" download class="btn btn-danger btn-sm btn-xs"><i class="fa fa-download" aria-hidden="true"></i></a>
                    <?php elseif ($user_role == 'operation' || $user_role == 'guest') : ?>
                        <span>Hubungi Asset untuk akses</span>
                    <?php endif; ?>
                <?php else: ?>
    ...
    document.getElementById("exportExcel").addEventListener("click", function () {
        ...
        XLSX.writeFile(wb, "BlowOutPreventer-ESS-2025.xlsx");
    });
    ...
    document.getElementById("exportCSV").addEventListener("click", function () {
        ...
        link.download = "BlowOutPreventer-ESS-2025.csv";
        link.click();
    });
    </script>
  ...
  <div class="modal fade" id="deltableModal" tabindex="-1" aria-labelledby="deltableModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="deltableModalLabel">RIWAYAT HAPUS - BOP</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
            ...
            <div class="table-responsive">
              <table id="dz" class="table table-striped table-tertiary-color cell-border hover text-white" style="width: 100%;">
                ...
                <tbody>
                  ...
                  $sql = "SELECT periode_laporan, rig_operation, rig_yard, sn_bop, jenis_bop, size, pressure, man, inputed_by, deleted_by, deletion_time FROM tb_dbdelbop";
    ...
    <div class="modal fade" id="edittableModal" tabindex="-1" aria-labelledby="edittableModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="edittableModalLabel">RIWAYAT EDIT - BOP</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <!-- Table Inside Modal -->
            <div class="table-responsive">
              <table id="ds" class="table table-striped table-tertiary-color cell-border hover text-white" style="width: 100%;">
                ...
                <tbody>
                  ...
                  $sql = "SELECT periode_laporan, rig_operation, rig_yard, new_rig_operation, new_rig_yard, sn_bop, jenis_bop, size, pressure, man, inputed_by, edited_by, edit_time FROM tb_dbeditbop";
                  $query = $conn->query($sql);
                  while ($row = $query->fetch_assoc()){
                    ?>
                    <tr class="align-middle text-center">
